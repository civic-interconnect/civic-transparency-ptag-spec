...
def test_series_schema_points_array() -> None:
    ...
    # Load series schema
    series_path = pkg.joinpath("series.schema.json")
    with open(str(series_path), "r", encoding="utf-8") as f:
        series_schema = json.load(f)

    # Load provenance_tag schema for cross-references
    provenance_path = pkg.joinpath("provenance_tag.schema.json")
    ...
    registry = Registry[Any]().with_resources(
        [
            ("series.schema.json", Resource.from_contents(series_schema)),
            ("provenance_tag.schema.json", Resource.from_contents(provenance_schema)),
        ]
    )
...
def test_series_schema_privacy_use_cases() -> None:
    ...
    registry: Registry[Any] = Registry[Any]().with_resources(
        [
            ("series.schema.json", Resource[Any].from_contents(series_schema)),
            (
                "provenance_tag.schema.json",

...
def test_openapi_31_validates_offline() -> None:
    ...
    res = files("ci.transparency.ptag.spec.schemas").joinpath(
...
def test_relative_series_schema_exists_next_to_openapi() -> None:
    """Focused guard to catch the './ptag_series.schema.json' problem immediately."""
    res = files("ci.transparency.ptag.spec.schemas").joinpath(
        "ptag_api.openapi.yaml"
    )
    with as_file(res) as p:
        ...
        base_dir = _uri_to_local_path(base_uri).parent
        target = (base_dir / "./ptag_series.schema.json").resolve()
        print(f"[neighbor] {target} exists={target.exists()}")
        assert target.exists(), "ptag_series.schema.json missing next to the OpenAPI file"
